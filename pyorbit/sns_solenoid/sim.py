...
latfile_solenoid = '_latfiles/SNSring_linear_noRF_nux6.18_nuy6.18_solenoids.lat'
latseq = 'rnginj'
fringe = True
...
env = Envelope(eps, mode, ex_frac, mass, kin_energy, length=lattice.getLength())
env.match_bare(lattice, '2D') # if '4D', beam will be flat for uncoupled lattice

# We want to start by matching to the bare lattice without solenoids, then
# observe their effect.
lattice = hf.lattice_from_file(latfile_solenoid, latseq, fringe)
