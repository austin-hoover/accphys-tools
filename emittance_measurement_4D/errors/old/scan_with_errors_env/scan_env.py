...
max_kin_energy_error = 0.010 # GeV
...
init_env = DanilovEnvelope(eps, mode, ex_frac, mass, kin_energy, length=bunch_length)
...
def initialize_env(init_twiss):
    ...
    env.set_twiss2D(ax0, ay0, bx0, by0, ex_frac)
    env.set_twiss4D_param('nu', np.radians(100))
    return env
        ...
        for quad_strengths in quad_strengths_list:
            ...
            for ws_name, monitor_node in zip(ws_names, env_monitor_nodes):
                ...
                if errors['beam moments']:
                    Sigma *= np.random.uniform(1 - max_frac_beam_moments_error, 
                                               1 + max_frac_beam_moments_error)
