...
# Simulation
#------------------------------------------------------------------------------
Sigmas_list, fail_rates, errors_included = [], [], []

keys = sorted(list(errors))

for i in trange(len(keys) + 1):
    
    if i < len(keys):
        # Turn off all but this error.
        for key in errors:
            errors[key] = False
        errors[keys[i]] = True
        errors_included.append(keys[i])
    else:
        # Use all errors.
        for key in errors:
            errors[key] = True
        errors_included.append('all')
    pprint(errors)
    
    for sc_node in sc_nodes:
        sc_node.setCalculationOn(errors['space charge'] and intensity > 0) 
...
#     print('-----------------------------------------')
    ...
    for i in trange(n_trials):
        ...
        Sigmas.append(Sigma)
#         print('{:<5} | {:.3f} | {:.3f} | {:.3f} | {:.3f}'.format(i + 1, *analysis.emittances(Sigma)))

    fail_rates.append(float(n_fail) / n_trials)
    Sigmas_list.append(Sigmas)
    print()
    

np.save('_output/data/Sigma0.npy', Sigma0)
np.save('_output/data/Sigmas_list.npy', Sigmas_list)
np.save('_output/data/fail_rates.npy', fail_rates)

file = open('_output/data/errors_included.txt', 'w')
for error_included in errors_included:
    file.write(error_included + '\n')
file.close()