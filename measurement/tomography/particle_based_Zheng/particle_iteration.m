...
pnum=6;%用于重建的截面数 no. of profiles used to reconstruct
% 
...
prodis=permute(prodis,[2 1 3]);%这是因为上面求解的分布是转置的，调整回来
%normalization
for i=1:pnum
...
load parnum.mat
% end
Q2=[6.46 	4.97 	3.99 	3.47 	4.32 	5.18   ];   %改变Q铁k值，截面1,6,8,10,13,16
...
prodispart=permute(prodispart,[2 1 3]);%这是因为上面求解的分布是转置的，调整回来
prorec=prodispart;
%归一化
for i=1:pnum
...
%生成新的dst文件 generate new input particle distribution for the next iteration
