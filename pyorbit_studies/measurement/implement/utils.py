...
class MadxController:
    ...
    def create_lattice(self, hide_output=False):
        """Run script and read .lat file to create TEAPOT_Lattice."""
...
class Scanner:
    ...
    def add_analysis_node(self, ws_name, kind='env_monitor', mm_mrad=False):
        """Add analysis node as child of WS node."""
        ws_node = self.lattice.getNodeForName(ws_name)
        position = self.lattice.getNodePositionsDict()[ws_node][0]
