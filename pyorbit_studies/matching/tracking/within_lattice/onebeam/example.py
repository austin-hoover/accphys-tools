...
# Initial beam
mode = 2
eps = 50e-6 # intrinsic emitance [m*rad]
ex_frac = 0.5 # ex/eps
...
perturb_radius = 0.1 # if nonzero, perturb the matched envelope
...
if match and intensity > 0:
    print 'Matching.'
    env.match(lattice, solver_nodes, tol=tol, verbose=verbose, method='lsq')
if perturb_radius > 0:
...
# Track envelope
def track_envelope(env):
    print 'Tracking envelope.'
    print '    turn |   cx   |   cy   |  phi'
    print '    --------------------------------'
    for t in range(nturns):
        env.track(lattice, nturns=1, ntestparts=ntestparts)
        cx, cy = 1000 * env.radii()
        phi = np.degrees(env.tilt_angle())
        print '    {}    | {:.3f} | {:.3f} | {:.3f}'.format(t, cx, cy, phi)
    
track_envelope(env)

print "Now try the 'custom' matching method. Note: I think the value of the cost function is the same as scipy.least_squares; I think there is a scaling factor missing."
env.match(lattice, solver_nodes, tol=tol, verbose=verbose, method='replace_by_avg')
track_envelope(env)
